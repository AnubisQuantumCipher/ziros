...
pub fn audit_program_with_capability_matrix(
    ...
    {
        let start = Instant::now();
        if let Some(cap) = underconstrained_dense_cell_cap_from_env() {
            ...
            if estimate > cap {
                ...
                report.add_check(AuditCheck {
                    ...
                    evidence: Some(format!(
                        "analysis skipped: dense linear rank estimate {estimate} exceeds cap {cap} from {UNDERCONSTRAINED_DENSE_CELL_CAP_ENV}"
                    )),
                    duration_ms: Some(elapsed),
                });
            } else {
...
fn underconstrained_dense_cell_cap_from_env() -> Option<usize> {
    env::var(UNDERCONSTRAINED_DENSE_CELL_CAP_ENV)
        .ok()
