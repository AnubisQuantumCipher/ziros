...
pub fn select_provider_routes(
    ...
    for (provider, locality, vars) in [
        ...
        let detected = vars.iter().find_map(|name| {
            env::var(name)
                .ok()
    ...
    if let Some(provider_override) = provider_override
        && routes
            .iter()
            .all(|route| route.provider != provider_override)
    {
        routes.insert(
            0,
            route(
...
mod tests {
    ...
    fn openai_api_route_uses_bearer_auth_from_environment() {
        ...
        assert_eq!(
            auth_bearer_for_route(&route).as_deref(),
    ...
    fn openai_api_model_can_be_overridden_per_route_selection() {
        ...
        let routes =
            select_provider_routes(None, "ad-hoc", Some("openai-api"), Some("gpt-5.2-codex"));
        let openai = routes
            .iter()
