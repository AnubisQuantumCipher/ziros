...
#[cfg(target_os = "macos")]
use zkf_metal::try_metal_pallas_msm;
...
/// Pallas-specialized MSM entrypoint for Nova's Pasta cycle.
///
...
pub fn msm_pallas(coeffs: &[PallasScalar], bases: &[PallasAffine]) -> Pallas {
  assert_eq!(coeffs.len(), bases.len());
  let n = coeffs.len();
  if n == 0 {
  ...
  let extract_u64_group =
    |start: usize, end: usize, negate: bool| -> (Vec<PallasAffine>, Vec<u64>) {
      classified[start..end]
        .iter()
        .map(|&v| {
          let idx = extract_index(v);
  ...
  let extract_binary_group = |start: usize, end: usize| -> Vec<PallasAffine> {
    classified[start..end]
      .iter()
      .map(|&v| bases[extract_index(v)])
      .collect()
  ...
  let (binary_result, small_and_large_result) = rayon::join(
    || {
      let (pos, neg) = rayon::join(
        || {
          let bases_pos = extract_binary_group(g0_start, g0_end);
          accumulate_bases::<PallasAffine>(&bases_pos)
        },
        || {
          let bases_neg = extract_binary_group(g1_start, g1_end);
          accumulate_bases::<PallasAffine>(&bases_neg)
        },
      );
      pos - neg
    },
    || {
      let (small_result, large_result) = rayon::join(
        ...
        || {
          ...
          let (large_bases, large_coeffs): (Vec<PallasAffine>, Vec<PallasScalar>) = classified
            [g10_start..g10_end]
            .iter()
            .map(|&v| {
              let idx = extract_index(v);
            ...
            .unzip();
          #[cfg(target_os = "macos")]
          if let Some(result) = try_metal_pallas_msm(&large_coeffs, &large_bases) {
            return result;
          }
          halo2curves::msm::msm_best(&large_coeffs, &large_bases)
        },
