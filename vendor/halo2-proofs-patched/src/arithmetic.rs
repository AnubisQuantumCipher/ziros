...
pub fn best_multiexp<C: CurveAffine>(coeffs: &[C::Scalar], bases: &[C]) -> C::Curve {
    ...
    #[cfg(target_os = "macos")]
    if let Some(result) = try_metal_eq_multiexp(coeffs, bases) {
        return result;
    }
...
#[cfg(target_os = "macos")]
fn try_metal_eq_multiexp<C: CurveAffine>(coeffs: &[C::Scalar], bases: &[C]) -> Option<C::Curve> {
    if core::any::type_name::<C>() != core::any::type_name::<pasta_curves::EqAffine>()
        || core::any::type_name::<C::Scalar>() != core::any::type_name::<pasta_curves::Fp>()
        || core::any::type_name::<C::Curve>() != core::any::type_name::<pasta_curves::Eq>()
    {
        return None;
    }

    let eq_coeffs = coeffs
        .iter()
        .map(|coeff| field_from_bytes::<pasta_curves::Fp>(coeff.to_repr().as_ref()))
        .collect::<Option<Vec<_>>>()?;
    let eq_bases = bases
        .iter()
        .map(|base| {
            let coords = base.coordinates();
            if bool::from(coords.is_none()) {
                Some(pasta_curves::Eq::identity().to_affine())
            } else {
                let coords = coords.unwrap();
                let x = field_from_bytes::<pasta_curves::Fq>(coords.x().to_repr().as_ref())?;
                let y = field_from_bytes::<pasta_curves::Fq>(coords.y().to_repr().as_ref())?;
                Option::<pasta_curves::EqAffine>::from(pasta_curves::EqAffine::from_xy(x, y))
            }
        })
        .collect::<Option<Vec<_>>>()?;
    let result = zkf_metal::try_metal_vesta_msm(&eq_coeffs, &eq_bases)?;
    ...
    let coords = coords.unwrap();
    let x = field_from_bytes::<C::Base>(coords.x().to_repr().as_ref())?;
    let y = field_from_bytes::<C::Base>(coords.y().to_repr().as_ref())?;
    let affine = Option::<C>::from(C::from_xy(x, y))?;
    Some(affine.into())
}

#[cfg(target_os = "macos")]
fn field_from_bytes<F: PrimeField>(bytes: &[u8]) -> Option<F> {
