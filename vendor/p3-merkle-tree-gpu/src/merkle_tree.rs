    ...
    MerkleTree<F, W, M, DIGEST_ELEMS>
{
    ...
    /// Construct a `MerkleTree` from pre-computed digest layers.
    ///
    /// This allows external code (e.g. GPU-accelerated Merkle builders) to
    /// compute digest layers independently and wrap them into a valid tree.
    ///
