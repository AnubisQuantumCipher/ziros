...
const ML_DSA_IMPLEMENTATION: &str = "libcrux-ml-dsa-0.0.8::ml_dsa_87";
...
impl LocalPeerIdentitySet {
    pub fn load_or_create(config: &SwarmConfig, label: &str) -> Result<Self, DistributedError> {
        ...
        let ml_dsa_path = config.identity_path.join(format!("{label}.mldsa87"));
        ...
        let (ml_dsa_signing_key, ml_dsa_public_key, ml_dsa_provenance) =
            load_or_create_ml_dsa_material(&ml_dsa_path, label, config.key_backend)?;
    ...
    pub fn rotate(config: &SwarmConfig, label: &str) -> Result<Self, DistributedError> {
        ...
        let (ml_dsa_signing_key, ml_dsa_public_key, ml_dsa_provenance) =
            regenerate_ml_dsa_material(&ml_dsa_path, label, config.key_backend)?;
        Ok(Self::from_material(
            ed25519_path,
            ml_dsa_path,
            label,
    ...
    pub fn sign_bundle(&self, bytes: &[u8]) -> SignatureBundle {
        let signing_key = load_ml_dsa_signing_key_for_backend(
            &self.ml_dsa_private_key_path,
            &self.label,
            self.backend_kind,
        )
        ...
        SignatureBundle {
            ...
            ml_dsa87: signature.as_slice().to_vec(),
        }
    }
    ...
    fn from_material(
        ...
        seed: [u8; 32],
        public_key_resynced: bool,
        _ml_dsa_signing_key: MLDSA87SigningKey,
        ml_dsa_public_key: MLDSA87VerificationKey,
        ml_dsa_provenance: MlDsaKeyProvenance,
    ) -> Self {
...
impl KeyStorageBackend for SecureEnclaveBackend {
    ...
    fn load_or_create(&self, path: &Path, label: &str) -> Result<LoadedSeed, DistributedError> {
        #[cfg(target_os = "macos")]
        {
            ensure_identity_dir(path)?;
            let key_manager =
                KeyManager::new().map_err(|err| DistributedError::Config(err.to_string()))?;
            let seed = match key_manager.retrieve_key(label, SWARM_ED25519_SERVICE) {
                Ok(bytes) => to_seed(&bytes)?,
                Err(_) => {
                    let seed = zkf_core::secure_random::secure_random_seed()
                        .map_err(DistributedError::Config)?;
                    key_manager
                        .store_key(label, SWARM_ED25519_SERVICE, &seed)
    ...
    fn load_existing(&self, path: &Path, label: &str) -> Result<LoadedSeed, DistributedError> {
        #[cfg(target_os = "macos")]
        {
            ...
            let seed = to_seed(
                &key_manager
                    .retrieve_key(label, SWARM_ED25519_SERVICE)
                    .map_err(|err| DistributedError::Config(err.to_string()))?,
            )?;
            let public_key_resynced =
    ...
    fn regenerate(&self, path: &Path, label: &str) -> Result<LoadedSeed, DistributedError> {
        #[cfg(target_os = "macos")]
        {
            ...
            let key_manager =
                KeyManager::new().map_err(|err| DistributedError::Config(err.to_string()))?;
            let _ = key_manager.delete_key(label, SWARM_ED25519_SERVICE);
            let seed =
                zkf_core::secure_random::secure_random_seed().map_err(DistributedError::Config)?;
            key_manager
                .store_key(label, SWARM_ED25519_SERVICE, &seed)
...
> {
    ...
    if backend_kind == SwarmKeyBackend::Enclave {
        #[cfg(target_os = "macos")]
        {
            let key_manager =
                KeyManager::new().map_err(|err| DistributedError::Config(err.to_string()))?;
            if let Ok(bytes) = key_manager.retrieve_key(label, SWARM_MLDSA87_SERVICE) {
                let signing_key: [u8; MLDSA87SigningKey::len()] = bytes.try_into().map_err(|_| {
                ...
                sync_ml_dsa_sidecars(path, label, backend_kind, &verification_key)?;
                return Ok((
                    MLDSA87SigningKey::new(signing_key),
...
> {
    ...
    if backend_kind == SwarmKeyBackend::Enclave {
        #[cfg(target_os = "macos")]
        {
            let key_manager =
                KeyManager::new().map_err(|err| DistributedError::Config(err.to_string()))?;
            let _ = key_manager.delete_key(label, SWARM_MLDSA87_SERVICE);
            key_manager
                .store_key(label, SWARM_MLDSA87_SERVICE, keypair.signing_key.as_slice())
                .map_err(|err| DistributedError::Config(err.to_string()))?;
        }
...
fn load_ml_dsa_signing_key(path: &Path) -> Result<MLDSA87SigningKey, DistributedError> {
...
fn load_ml_dsa_signing_key_for_backend(
    path: &Path,
    label: &str,
    backend_kind: SwarmKeyBackend,
) -> Result<MLDSA87SigningKey, DistributedError> {
    if backend_kind == SwarmKeyBackend::Enclave {
        #[cfg(target_os = "macos")]
        {
            let key_manager =
            ...
            let bytes = key_manager
                .retrieve_key(label, SWARM_MLDSA87_SERVICE)
                .map_err(|err| DistributedError::Config(err.to_string()))?;
            let signing_key: [u8; MLDSA87SigningKey::len()] = bytes.try_into().map_err(|_| {
                DistributedError::Config(
                    "swarm ML-DSA key material is corrupt; run `zkf swarm regenerate-key --force`"
            ...
            return Ok(MLDSA87SigningKey::new(signing_key));
        }
    }
    load_ml_dsa_signing_key(path)
}

fn load_ml_dsa_public_key(path: &Path) -> Result<MLDSA87VerificationKey, DistributedError> {
    let bytes = fs::read(path)?;
    let verification_key: [u8; MLDSA87VerificationKey::len()] = bytes.try_into().map_err(|_| {
        DistributedError::Config(
...
fn ml_dsa_provenance(
    ...
    MlDsaKeyProvenance {
        ...
        storage_path: if backend_kind == SwarmKeyBackend::Enclave {
            format!("keychain://{SWARM_MLDSA87_SERVICE}/{label}")
        } else {
        ...
        permissions: if backend_kind == SwarmKeyBackend::Enclave {
            "keychain".to_string()
        } else {
            "0600".to_string()
        },
...
fn sync_ml_dsa_sidecars(
    ...
    fs::write(
        ...
        serde_json::to_vec_pretty(&ml_dsa_provenance(path, label, backend_kind))
...
fn ensure_backend_policy(config: &SwarmConfig) -> Result<(), DistributedError> {
    #[cfg(target_os = "macos")]
    {
        ...
        if policy_mode != "observe"
            && config.key_backend != SwarmKeyBackend::Enclave
            && zkf_core::keystore::synchronizable_keychain_supported()
        {
...
fn ml_dsa_metadata_path(path: &Path) -> PathBuf {
    let mut public = path.to_path_buf();
    public.set_extension("mldsa87.meta.json");
    public
}
...
mod tests {
    ...
    fn rotate_rewrites_public_key_sidecars() {
        with_temp_home(|_| {
            ...
            let stored_ml_dsa = fs::read(ml_dsa_public_key_path(
                &config.identity_path.join("rotate-me.mldsa87"),
            ))
            .unwrap();
    ...
    fn mldsa_metadata_sidecar_is_written() {
        with_temp_home(|_| {
            ...
            let metadata_path =
                ml_dsa_metadata_path(&config.identity_path.join("sidecars.mldsa87"));
