...
// ============================================================================
// Vesta MSM: Unified shader for Multi-Scalar Multiplication
// Includes: Fp field arithmetic, G1 projective curve ops, Pippenger kernels
//
// Vesta curve: y² = x³ + 5 over Fq (a=0, same Jacobian point ops as BN254)
// ============================================================================

// VESTA BASE FIELD (Fq) — used for point coordinates
// q = 0x40000000000000000000000000000000224698fc0994a8dd8c46eb2100000001
// Montgomery form: all field elements stored as a * R mod q, where R = 2^256

struct Fp {
    uint64_t limbs[4];
};

// Fq modulus (little-endian limbs)
constant Fp FQ_P = {{
    0x8c46eb2100000001ULL,
    0x224698fc0994a8ddULL,
...
constant Fp FQ_ONE = {{
    0x5b2b3e9cfffffffdULL,
    0x992c350be3420567ULL,
    0xffffffffffffffffULL,
    0x3fffffffffffffffULL
}};

// -q^(-1) mod 2^64
constant uint64_t FQ_INV = 0x8c46eb20ffffffffULL;

// R^2 mod q (for converting to Montgomery form)
constant Fp FQ_R2 = {{
    0xfc9678ff0000000fULL,
    0x67bb433d891a16e3ULL,
    0x7fae231004ccf590ULL,
    0x096d41af7ccfdaa9ULL
}};
...
static inline Fp fp_mul(Fp a, Fp b) {
    // 4-limb schoolbook multiply with interleaved Montgomery reduction (CIOS)
...
// Identity: Z = 0
// Curve: y² = x³ + 5 (a = 0, same point arithmetic as BN254)
...
kernel void msm_bucket_assign(
    device const uint64_t *scalars [[buffer(0)]],
    device uint32_t *point_bucket_map [[buffer(1)]],
    ...
    uint tid [[thread_position_in_grid]])
{
    ...
    for (uint32_t w = 0; w < num_windows; w++) {
        uint32_t bucket_idx = extract_window(s, w, c);
    ...
    ushort simd_size [[threads_per_simdgroup]])
{
    ...
    // Reduce across SIMD lanes via u32 shuffles
...
// Constant zero for Y-negation in NAF kernel
constant Fp FP_ZERO = {{0, 0, 0, 0}};

// NAF (Non-Adjacent Form) bucket accumulation for Pallas.
// Same as msm_bucket_acc but reads NAF-encoded map entries where
// bit 31 = sign, bits [0..30] = absolute bucket index.
// Point negation on Pallas: -P = (X, -Y), i.e. negate Y coordinate.
kernel void msm_bucket_acc_naf(
    device const uint64_t *bases_x [[buffer(0)]],
    device const uint64_t *bases_y [[buffer(1)]],
    device const uint32_t *naf_map [[buffer(2)]],
    device uint64_t *bucket_results [[buffer(3)]],
    constant uint32_t &n_points [[buffer(4)]],
    constant uint32_t &c [[buffer(5)]],
    constant uint32_t &num_windows [[buffer(6)]],
    constant uint32_t &num_buckets_naf [[buffer(7)]],
    uint tid [[thread_position_in_grid]])
{
    uint32_t total_buckets = num_windows * num_buckets_naf;
    if (tid >= total_buckets) return;

    uint32_t window = tid / num_buckets_naf;
    uint32_t bucket = tid % num_buckets_naf;

    // Bucket 0 = identity (points with zero NAF digit)
    if (bucket == 0) {
        uint32_t out = tid * 12;
        for (int i = 0; i < 12; i++) bucket_results[out + i] = 0;
        return;
    ...
    for (uint32_t i = 0; i < n_points; i++) {
        uint32_t entry = naf_map[i * num_windows + window];
        uint32_t abs_bucket = entry & 0x7FFFFFFFu;
        uint32_t sign = entry >> 31;

        if (abs_bucket == bucket) {
            Fp px, py;
            for (int j = 0; j < 4; j++) {
                px.limbs[j] = bases_x[i * 4 + j];
                py.limbs[j] = bases_y[i * 4 + j];
            ...
            // Negate Y if sign bit is set: -P = (X, -Y) on Pallas
            if (sign) {
