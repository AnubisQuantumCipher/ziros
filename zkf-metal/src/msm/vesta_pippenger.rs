...
pub fn metal_vesta_msm(ctx: &MetalContext, scalars: &[Fp], bases: &[EqAffine]) -> Option<Eq> {
    ...
    let msm_lib = ctx.vesta_msm_library()?;
    ...
    let mut scalar_data: Vec<u64> = scalars
        ...
        .zip(bases.iter())
        .flat_map(|(s, b)| {
            if vesta::is_identity(b) {
                [0u64; 4].into_iter()
            } else {
                vesta::scalar_to_limbs(s).into_iter()
            }
        })
        .collect();
    ...
    for base in bases {
        if vesta::is_identity(base) {
            ...
            let (x_limbs, y_limbs) = vesta::affine_to_limbs(base);
    ...
    Some(vesta::combine_windows(&window_results, c))
}

/// Execute NAF-encoded Pippenger MSM on Metal GPU for Vesta curve.
///
...
pub fn metal_vesta_msm_naf(ctx: &MetalContext, scalars: &[Fp], bases: &[EqAffine]) -> Option<Eq> {
    ...
    let num_windows = vesta::num_windows(c);
    let num_buckets = vesta::num_buckets_naf(c) as usize;
    let total_buckets = num_windows as usize * num_buckets;
    launch_contracts::msm_contract(MsmContractInput {
    ...
    for (scalar, base) in scalars.iter().zip(bases.iter()) {
        if vesta::is_identity(base) {
            ...
            let windows = vesta::scalar_to_naf_windows(&limbs, c, num_windows);
...
pub fn cpu_vesta_pippenger(scalars: &[Fp], bases: &[EqAffine]) -> Eq {
    ...
    vesta::combine_windows(&window_results, c)
}

/// Try Metal GPU MSM for Vesta curve.
///
...
mod tests {
    ...
    fn cpu_vesta_pippenger_small() {
        ...
        assert_eq!(
            result.to_affine(),
            expected.to_affine(),
            "CPU Vesta Pippenger mismatch"
        );
    }

    #[test]
    fn metal_vesta_msm_matches_cpu() {
        ...
        match gpu_result {
            Some(gpu) => {
                assert_eq!(
                    ...
                    "Metal Vesta MSM result does not match CPU reference"
                );
                eprintln!("  [vesta-msm] n={n}: GPU == CPU ✓");
            }
            None => {
                eprintln!("  [vesta-msm] Metal MSM dispatch failed, skipping");
            }
        }
    }

    #[test]
    fn metal_vesta_msm_with_identities() {
        let ctx = match crate::device::global_context() {
            Some(c) => c,
            None => {
                eprintln!("No Metal GPU available, skipping");
        ...
        match gpu_result {
            Some(gpu) => {
                assert_eq!(
                    gpu.to_affine(),
                    cpu_result.to_affine(),
                    "Metal Vesta MSM with identities: mismatch"
                );
                eprintln!("  [vesta-msm] identities n={n}: GPU == CPU ✓");
            }
            None => {
                eprintln!("  [vesta-msm] identity test: GPU dispatch failed");
            }
        }
    }

    #[test]
    fn metal_vesta_msm_randomized_batches_match_cpu() {
        let ctx = match crate::device::global_context() {
            Some(c) => c,
            None => {
                eprintln!("No Metal GPU available, skipping");
        ...
        for seed in [11u64, 29u64] {
            ...
            let cpu_result = cpu_vesta_pippenger(&scalars, &bases);
            let gpu_result = metal_vesta_msm(ctx, &scalars, &bases)
                .expect("Metal Vesta MSM dispatch should succeed for randomized batch");

            assert_eq!(
                gpu_result.to_affine(),
                cpu_result.to_affine(),
                "randomized Metal Vesta MSM mismatch for seed {seed}"
            );
        }
    }
}
