...
__kernel void msm_bucket_reduce(
    __global ulong* accumulators,
    const uint count
) {
    size_t gid = get_global_id(0);
    if (gid + 1 < count) {
        accumulators[gid] = accumulators[gid] + accumulators[gid + 1];
    }
}

__kernel void msm_window_combine(
    __global ulong* accumulators,
    const uint count
) {
    size_t gid = get_global_id(0);
    if (gid < count) {
        accumulators[gid] = (accumulators[gid] << 1) ^ (ulong) gid;
    }
}
