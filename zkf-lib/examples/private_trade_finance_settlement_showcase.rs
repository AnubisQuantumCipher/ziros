...
fn output_dir() -> PathBuf {
    env::args_os().nth(1).map(PathBuf::from).unwrap_or_else(|| {
        PathBuf::from(env::var("HOME").unwrap_or_else(|_| ".".to_string()))
            .join("Desktop/ZirOS_Private_Trade_Finance_Settlement")
    })
}
...
fn primary_backend_selection() -> ZkfResult<zkf_backends::BackendSelection> {
    let requested = env::var("ZKF_PRIVATE_TRADE_FINANCE_SETTLEMENT_PRIMARY_BACKEND")
    ...
    if !matches!(selection.backend, BackendKind::HyperNova) {
        return Err(ZkfError::Backend(format!(
            "trade finance showcase requires hypernova primary backend, got {}",
...
fn run_export() -> ZkfResult<PathBuf> {
    ...
    run_private_trade_finance_settlement_export(PrivateTradeFinanceSettlementExportConfig {
        out_dir,
        profile: export_profile()?,
        primary_backend: primary_backend_selection()?,
        distributed_mode_requested: env_flag("ZKF_PRIVATE_TRADE_FINANCE_SETTLEMENT_DISTRIBUTED"),
    })
}

fn main() -> ZkfResult<()> {
    if env_flag("ZKF_PRIVATE_TRADE_FINANCE_SETTLEMENT_DIAGNOSTICS") {
        let diagnostics = run_with_large_stack_result(
            "private-trade-finance-diagnostics",
            run_private_trade_finance_settlement_hypernova_diagnostics,
        )?;
        println!(
            "{}",
