...
pub const SOVEREIGN_ECONOMIC_DEFENSE2_GOLDILOCKS_SCALE_DECIMALS: u32 = 3;
pub const SOVEREIGN_ECONOMIC_DEFENSE2_BN254_SCALE_DECIMALS: u32 = 18;
pub const SOVEREIGN_ECONOMIC_DEFENSE2_INTEGRATION_STEPS: usize = 96;

const SED2_GOLDILOCKS_FIELD: FieldId = FieldId::Goldilocks;
const SED2_BN254_FIELD: FieldId = FieldId::Bn254;
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(deny_unknown_fields)]
pub struct SovereignEconomicDefense2RunManifestV1 {
    pub run_id: String,
...
fn sed2_goldilocks_scale() -> BigInt {
    fixed_scale(SOVEREIGN_ECONOMIC_DEFENSE2_GOLDILOCKS_SCALE_DECIMALS)
}

fn sed2_bn254_scale() -> BigInt {
    fixed_scale(SOVEREIGN_ECONOMIC_DEFENSE2_BN254_SCALE_DECIMALS)
}

fn sed2_goldilocks_amount_bound() -> BigInt {
    BigInt::from(1_000_000_000u64)
}

fn sed2_goldilocks_score_bound() -> BigInt {
    BigInt::from(1_000_000u64)
}

fn sed2_bn254_amount_bound() -> BigInt {
    fixed_scale(SOVEREIGN_ECONOMIC_DEFENSE2_BN254_SCALE_DECIMALS) * BigInt::from(1_000_000u64)
}
...
fn parse_goldilocks_amount(value: &str, label: &str) -> ZkfResult<BigInt> {
    let parsed = decimal_scaled(value, SOVEREIGN_ECONOMIC_DEFENSE2_GOLDILOCKS_SCALE_DECIMALS);
    ensure_nonnegative_le(label, &parsed, &sed2_goldilocks_amount_bound())?;
    Ok(parsed)
}

fn parse_goldilocks_ratio(value: &str, label: &str) -> ZkfResult<BigInt> {
    let parsed = decimal_scaled(value, SOVEREIGN_ECONOMIC_DEFENSE2_GOLDILOCKS_SCALE_DECIMALS);
    ensure_nonnegative_le(label, &parsed, &sed2_goldilocks_scale())?;
    Ok(parsed)
}

fn parse_bn254_amount(value: &str, label: &str) -> ZkfResult<BigInt> {
    let parsed = decimal_scaled(value, SOVEREIGN_ECONOMIC_DEFENSE2_BN254_SCALE_DECIMALS);
    ensure_nonnegative_le(label, &parsed, &sed2_bn254_amount_bound())?;
    Ok(parsed)
}

fn parse_bn254_ratio(value: &str, label: &str) -> ZkfResult<BigInt> {
    let parsed = decimal_scaled(value, SOVEREIGN_ECONOMIC_DEFENSE2_BN254_SCALE_DECIMALS);
    ensure_nonnegative_le(label, &parsed, &sed2_bn254_scale())?;
    Ok(parsed)
}
...
pub fn build_cooperative_treasury_assurance_program(
    ...
    let amount_bits = bits_for_bound(&sed2_goldilocks_amount_bound());
    let ratio_bits = bits_for_bound(&sed2_goldilocks_scale());

    let mut builder = ProgramBuilder::new(
        format!("sovereign_economic_defense2_cooperative_treasury_{members}"),
        SED2_GOLDILOCKS_FIELD,
    );
    builder.metadata_entry("application", "sovereign-economic-defense2")?;
    builder.metadata_entry("circuit", "cooperative-treasury-assurance")?;
    builder.metadata_entry("members", members.to_string())?;
    builder.metadata_entry("fixed_point_scale", sed2_goldilocks_scale().to_str_radix(10))?;
    ...
    builder.append_exact_division_constraints(
        mul_expr(
            ...
            const_expr(&sed2_goldilocks_scale()),
        ),
        signal_expr("cta_total_contributions"),
        "cta_reserve_ratio",
        "cta_reserve_ratio_remainder",
        "cta_reserve_ratio_slack",
        &sed2_goldilocks_amount_bound(),
        "cta_reserve_ratio",
    )?;
    ...
    append_geq_comparator_bit(
        ...
        "cta_buffer_ok",
        "cta_buffer_ok_slack",
        &positive_comparison_offset(&sed2_goldilocks_scale()),
        "cta_buffer_ok",
    )?;
    ...
    builder.add_lookup_table(
        ...
        2,
        vec![
            vec![FieldElement::ZERO, field_ref(&sed2_goldilocks_scale())],
            vec![
                FieldElement::ONE,
                field_ref(&(sed2_goldilocks_scale() / two())),
            ],
        ],
    )?;
...
pub fn cooperative_treasury_assurance_witness_from_request(
    ...
    let members = request.contributions.len();
    let scale = sed2_goldilocks_scale();
...
pub fn build_community_land_trust_governance_program(
    ...
    let mut builder = ProgramBuilder::new(
        format!("sovereign_economic_defense2_clt_governance_{households}"),
        SED2_GOLDILOCKS_FIELD,
    );
    builder.metadata_entry("application", "sovereign-economic-defense2")?;
    ...
    append_geq_comparator_bit(
        ...
        &positive_comparison_offset(&sed2_goldilocks_amount_bound()),
        "clt_reserve_buffer_ok",
    )?;
    ...
    builder.constrain_select(
        ...
        add_expr(vec![
            ...
            const_expr(&(sed2_goldilocks_scale() / BigInt::from(20u8))),
        ]),
        signal_expr("clt_required_occupancy_rate"),
    )?;
    builder.append_exact_division_constraints(
        mul_expr(signal_expr("clt_occupied_units"), const_expr(&sed2_goldilocks_scale())),
    ...
    for index in 0..households {
        ...
        builder.append_exact_division_constraints(
            mul_expr(signal_expr(&equity_names[index]), const_expr(&sed2_goldilocks_scale())),
...
pub fn build_anti_extraction_shield_program(
    ...
    let amount_bits = bits_for_bound(&sed2_bn254_amount_bound());
    let ratio_bits = bits_for_bound(&sed2_bn254_scale());

    let mut builder = ProgramBuilder::new(
        format!("sovereign_economic_defense2_anti_extraction_{payment_count}"),
        SED2_BN254_FIELD,
    );
    builder.metadata_entry("application", "sovereign-economic-defense2")?;
    builder.metadata_entry("circuit", "anti-extraction-shield")?;
    builder.metadata_entry("payment_count", payment_count.to_string())?;
    builder.metadata_entry("fixed_point_scale", sed2_bn254_scale().to_str_radix(10))?;
    ...
    builder.add_lookup_table(
        ...
        vec![
            vec![
                ...
                field(BigInt::from(0u8)),
                field(decimal_scaled("0.18", SOVEREIGN_ECONOMIC_DEFENSE2_BN254_SCALE_DECIMALS)),
            ],
            vec![
                field(BigInt::from(0u8)),
                field(BigInt::from(1u8)),
                field(decimal_scaled("0.21", SOVEREIGN_ECONOMIC_DEFENSE2_BN254_SCALE_DECIMALS)),
            ],
            vec![
                field(BigInt::from(1u8)),
                field(BigInt::from(0u8)),
                field(decimal_scaled("0.24", SOVEREIGN_ECONOMIC_DEFENSE2_BN254_SCALE_DECIMALS)),
            ],
            vec![
                field(BigInt::from(1u8)),
                field(BigInt::from(1u8)),
                field(decimal_scaled("0.30", SOVEREIGN_ECONOMIC_DEFENSE2_BN254_SCALE_DECIMALS)),
            ],
        ],
    )?;
    builder.constrain_lookup(
    ...
    append_geq_comparator_bit(
        ...
        &positive_comparison_offset(&sed2_bn254_scale()),
        "aes_apr_violation",
    )?;
    builder.constrain_select(
    ...
    for (signal, bound) in [
        ("aes_apr_violation_mag", sed2_bn254_scale()),
        ("aes_dti_violation_mag", sed2_bn254_scale()),
        ("aes_term_violation_mag", BigInt::from(65_535u64)),
        ("aes_balloon_violation_mag", sed2_bn254_scale()),
    ] {
        builder.append_nonnegative_bound(signal, &bound, signal)?;
    }
    for (signal, bound) in [
        ("aes_effective_apr", sed2_bn254_scale()),
        ("aes_debt_to_income_ratio", sed2_bn254_scale()),
        ("aes_apr_violation_mag", sed2_bn254_scale()),
        ("aes_dti_violation_mag", sed2_bn254_scale()),
        ("aes_term_violation_mag", BigInt::from(65_535u64)),
        ("aes_balloon_violation_mag", sed2_bn254_scale()),
    ...
    builder.append_exact_division_constraints(
        const_expr(&(&sed2_bn254_scale() * &sed2_bn254_scale())),
...
pub fn anti_extraction_shield_witness_from_request(
    ...
    let lookup_apr_ceiling = match (request.loan_type_code, request.term_bucket) {
        (0, 0) => decimal_scaled("0.18", SOVEREIGN_ECONOMIC_DEFENSE2_BN254_SCALE_DECIMALS),
        (0, 1) => decimal_scaled("0.21", SOVEREIGN_ECONOMIC_DEFENSE2_BN254_SCALE_DECIMALS),
        (1, 0) => decimal_scaled("0.24", SOVEREIGN_ECONOMIC_DEFENSE2_BN254_SCALE_DECIMALS),
        (1, 1) => decimal_scaled("0.30", SOVEREIGN_ECONOMIC_DEFENSE2_BN254_SCALE_DECIMALS),
        _ => {
            return Err(ZkfError::InvalidArtifact(
...
pub fn build_wealth_trajectory_assurance_program(
    ...
    let mut builder = ProgramBuilder::new(
        format!("sovereign_economic_defense2_wealth_trajectory_{assets}"),
        SED2_GOLDILOCKS_FIELD,
    );
    builder.metadata_entry("application", "sovereign-economic-defense2")?;
...
pub fn build_recirculation_sovereignty_score_program(
    ...
    if request.internal_spend.len() != SOVEREIGN_ECONOMIC_DEFENSE2_INTEGRATION_STEPS
        || request.external_spend.len() != SOVEREIGN_ECONOMIC_DEFENSE2_INTEGRATION_STEPS
    {
        return Err(ZkfError::InvalidArtifact(format!(
            "recirculation sovereignty score requires exactly {} internal and external flow steps",
            SOVEREIGN_ECONOMIC_DEFENSE2_INTEGRATION_STEPS
        )));
    }
    let steps = SOVEREIGN_ECONOMIC_DEFENSE2_INTEGRATION_STEPS;
    let scale = sed2_goldilocks_scale();
    let amount_bits = bits_for_bound(&sed2_goldilocks_amount_bound());
    let ratio_bits = bits_for_bound(&scale);

    let mut builder = ProgramBuilder::new(
        format!("sovereign_economic_defense2_recirculation_sovereignty_{steps}"),
        SED2_GOLDILOCKS_FIELD,
    );
    builder.metadata_entry("application", "sovereign-economic-defense2")?;
...
mod tests {
    ...
    const SOVEREIGN_ECONOMIC_DEFENSE2_TEST_STACK_SIZE: usize = 128 * 1024 * 1024;
        ...
        F: FnOnce() + Send + 'static,
    {
        let handle = thread::Builder::new()
            .name(name.to_string())
            .stack_size(SOVEREIGN_ECONOMIC_DEFENSE2_TEST_STACK_SIZE)
            .spawn(test)
    ...
    #[test]
    fn sovereign_economic_defense2_circuit_roundtrip() {
