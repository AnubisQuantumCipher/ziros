...
pub fn template_registry() -> Vec<TemplateRegistryEntryV1> {
    vec![
        ...
        TemplateRegistryEntryV1 {
            id: "private-reentry-thermal-envelope".to_string(),
            description:
                "Theorem-first reusable launch vehicle reentry mission-assurance certificate with private trajectory commitments."
                    .to_string(),
            release_ready: true,
            template_args: vec![TemplateArgSpecV1 {
                name: "steps".to_string(),
                description: "Number of fixed reduced-order reentry steps.".to_string(),
                default_value: Some(
                    super::templates::PRIVATE_REENTRY_THERMAL_DEFAULT_STEPS.to_string(),
                ),
                required: false,
            }],
        },
...
pub fn instantiate_template(
    ...
    let template = match id {
        ...
        "private-reentry-thermal-envelope" => {
            reject_unknown_args(template_args, &["steps"])?;
            super::templates::private_reentry_thermal_showcase_with_steps(parse_usize_arg(
                template_args,
                "steps",
                super::templates::PRIVATE_REENTRY_THERMAL_DEFAULT_STEPS,
            )?)?
        }
...
mod tests {
    ...
    fn template_registry_contains_release_builder_entries() {
        ...
        assert!(
            registry
                .iter()
                .any(|entry| entry.id == "private-reentry-thermal-envelope")
        );
