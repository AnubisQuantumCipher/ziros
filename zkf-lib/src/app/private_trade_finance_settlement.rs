...
use zkf_core::{
    generate_witness, BigIntFieldValue, Expr, FieldElement, FieldId, FieldValue, Program, Witness,
...
pub const PRIVATE_TRADE_FINANCE_MAX_PERILS: usize = 4;
...
const TRADE_FINANCE_DOMAIN_FRAUD: i64 = 1103;
const TRADE_FINANCE_DOMAIN_PAYOUT: i64 = 1104;
...
pub struct TradeFinancePolicyDataV1 {
    ...
    pub covered_peril_flags: [u64; PRIVATE_TRADE_FINANCE_MAX_PERILS],
    pub exclusion_flags: [u64; PRIVATE_TRADE_FINANCE_MAX_PERILS],
...
#[derive(Debug, Clone, Serialize, Deserialize, Eq, PartialEq)]
#[serde(deny_unknown_fields)]
pub struct TradeFinanceClaimEventDataV1 {
    ...
    pub peril_classification_flags: [u64; PRIVATE_TRADE_FINANCE_MAX_PERILS],
    pub damaged_asset_class: u64,
    pub claimed_loss_categories: [u64; PRIVATE_TRADE_FINANCE_MAX_PERILS],
    pub prior_claim_linkage_hashes: [u64; PRIVATE_TRADE_FINANCE_MAX_DIGESTS],
}
...
pub struct TradeFinanceEvidenceDataV1 {
    ...
    pub replacement_cost_schedules: [u64; PRIVATE_TRADE_FINANCE_MAX_LINE_ITEMS],
    pub depreciation_basis_values: [u64; PRIVATE_TRADE_FINANCE_MAX_LINE_ITEMS],
    pub telematics_structured_event_summary_values: [u64; PRIVATE_TRADE_FINANCE_MAX_DIGESTS],
...
pub struct TradeFinanceConsistencyFraudInputsV1 {
    pub duplicate_claim_candidate_hashes: [u64; PRIVATE_TRADE_FINANCE_MAX_DIGESTS],
...
pub struct TradeFinancePrivateInputsV1 {
    pub policy: TradeFinancePolicyDataV1,
    pub claim_event: TradeFinanceClaimEventDataV1,
    pub evidence: TradeFinanceEvidenceDataV1,
    pub analysis_inputs: TradeFinanceConsistencyFraudInputsV1,
    pub settlement_governance: TradeFinanceSettlementGovernanceInputsV1,
}
...
#[derive(Debug, Clone)]
pub struct ClaimsCoreComputation {
...
#[derive(Debug, Clone)]
pub struct ClaimsSettlementComputation {
    settlement_instruction_commitment: BigInt,
...
#[derive(Debug, Clone)]
pub struct ClaimsDisclosureComputation {
    role_code: u64,
...
#[derive(Debug, Clone)]
pub struct ClaimsShardComputation {
...
fn policy_input_name(name: &str) -> String {
    format!("trade_finance_policy_{name}")
}

fn policy_array_name(name: &str, index: usize) -> String {
    format!("trade_finance_policy_{name}_{index}")
}

fn claim_input_name(name: &str) -> String {
    format!("trade_finance_claim_{name}")
}

fn claim_array_name(name: &str, index: usize) -> String {
    format!("trade_finance_claim_{name}_{index}")
}
...
fn analysis_name(name: &str) -> String {
    format!("trade_finance_analysis_{name}")
}

fn analysis_array_name(name: &str, index: usize) -> String {
    format!("trade_finance_analysis_{name}_{index}")
}

fn governance_name(name: &str) -> String {
    format!("trade_finance_governance_{name}")
}

fn governance_array_name(name: &str, index: usize) -> String {
    format!("trade_finance_governance_{name}_{index}")
}

fn settlement_input_names() -> Vec<String> {
    vec![
        "trade_finance_settlement_claim_packet_commitment".to_string(),
        "trade_finance_settlement_coverage_decision_commitment".to_string(),
        "trade_finance_settlement_fraud_evidence_score".to_string(),
        "trade_finance_settlement_payout_amount".to_string(),
        "trade_finance_settlement_reserve_amount".to_string(),
        "trade_finance_settlement_reinsurer_share_amount".to_string(),
        "trade_finance_settlement_action_class_code".to_string(),
        "trade_finance_settlement_claimant_destination_commitment".to_string(),
        "trade_finance_settlement_insurer_reserve_account_commitment".to_string(),
        "trade_finance_settlement_reinsurer_participation_commitment".to_string(),
        ...
        "trade_finance_settlement_policy_effective_timestamp".to_string(),
        "trade_finance_settlement_policy_expiration_timestamp".to_string(),
        "trade_finance_settlement_incident_timestamp".to_string(),
        "trade_finance_settlement_reported_timestamp".to_string(),
    ]
}
...
fn disclosure_input_names() -> Vec<String> {
    vec![
        ...
        "trade_finance_disclosure_role_reinsurer".to_string(),
        "trade_finance_disclosure_role_claimant".to_string(),
        "trade_finance_disclosure_role_investigator".to_string(),
        "trade_finance_disclosure_claim_packet_commitment".to_string(),
        "trade_finance_disclosure_coverage_decision_commitment".to_string(),
        "trade_finance_disclosure_consistency_score_commitment".to_string(),
        "trade_finance_disclosure_fraud_score_commitment".to_string(),
        "trade_finance_disclosure_payout_commitment".to_string(),
        "trade_finance_disclosure_reserve_commitment".to_string(),
        "trade_finance_disclosure_settlement_commitment".to_string(),
        "trade_finance_disclosure_reinsurer_share_amount".to_string(),
...
fn shard_input_names() -> Vec<String> {
    ...
    for index in 0..PRIVATE_TRADE_FINANCE_MAX_DIGESTS {
        names.push(format!("trade_finance_shard_claim_commitment_{index}"));
    }
    names
}
...
pub fn trade_finance_private_input_names_v1() -> Vec<String> {
    ...
    for index in 0..PRIVATE_TRADE_FINANCE_MAX_PERILS {
...
pub fn build_trade_finance_decision_core_program() -> ZkfResult<Program> {
    let mut builder = ProgramBuilder::new(
        "trade_finance_trade_finance_decision_core_v1",
        TRADE_FINANCE_FIELD,
    );
    ...
    let claim_packet_digest = append_private_input_anchor_chain(
        &mut builder,
        &all_inputs,
    ...
    append_geq_comparator_bit(
        ...
        "trade_finance_within_period_incident_after_effective_bit",
        "trade_finance_within_period_incident_after_effective_slack",
        TRADE_FINANCE_TIMESTAMP_BOUND,
        "trade_finance_within_period_incident_after_effective",
    )?;
    append_geq_comparator_bit(
        &mut builder,
        signal_expr(&policy_input_name("expiration_timestamp")),
        signal_expr(&claim_input_name("incident_timestamp")),
        "trade_finance_within_period_expiration_after_incident_bit",
        "trade_finance_within_period_expiration_after_incident_slack",
        TRADE_FINANCE_TIMESTAMP_BOUND,
        "trade_finance_within_period_expiration_after_incident",
    )?;
    append_boolean_and(
        &mut builder,
        "trade_finance_within_period_bit",
        signal_expr("trade_finance_within_period_incident_after_effective_bit"),
        signal_expr("trade_finance_within_period_expiration_after_incident_bit"),
    )?;

    builder.private_signal("trade_finance_covered_peril_count")?;
    builder.bind(
        "trade_finance_covered_peril_count",
        add_expr(
            (0..PRIVATE_TRADE_FINANCE_MAX_PERILS)
                .map(|index| {
                    mul_expr(
    ...
    builder.constrain_range("trade_finance_covered_peril_count", 4)?;
    append_geq_comparator_bit(
        &mut builder,
        signal_expr("trade_finance_covered_peril_count"),
        const_expr(1),
        "trade_finance_covered_peril_supported_bit",
        "trade_finance_covered_peril_supported_slack",
        PRIVATE_TRADE_FINANCE_MAX_PERILS as u64 + 1,
        "trade_finance_covered_peril_supported",
    )?;

    builder.private_signal("trade_finance_excluded_peril_count")?;
    builder.bind(
        "trade_finance_excluded_peril_count",
        add_expr(
            (0..PRIVATE_TRADE_FINANCE_MAX_PERILS)
                .map(|index| {
                    mul_expr(
    ...
    builder.constrain_range("trade_finance_excluded_peril_count", 4)?;
    append_geq_comparator_bit(
        &mut builder,
        signal_expr("trade_finance_excluded_peril_count"),
        const_expr(1),
        "trade_finance_peril_excluded_bit",
        "trade_finance_peril_excluded_slack",
        PRIVATE_TRADE_FINANCE_MAX_PERILS as u64 + 1,
        "trade_finance_peril_excluded",
    )?;

    builder.private_signal("trade_finance_claimed_category_count")?;
    builder.bind(
        "trade_finance_claimed_category_count",
        add_expr(
            (0..PRIVATE_TRADE_FINANCE_MAX_PERILS)
    ...
    builder.constrain_range("trade_finance_claimed_category_count", 4)?;
    append_geq_comparator_bit(
        &mut builder,
        signal_expr("trade_finance_claimed_category_count"),
        const_expr(1),
        "trade_finance_claim_category_present_bit",
        "trade_finance_claim_category_present_slack",
        PRIVATE_TRADE_FINANCE_MAX_PERILS as u64 + 1,
        "trade_finance_claim_category_present",
    )?;

    builder.private_signal("trade_finance_not_peril_excluded_bit")?;
    builder.bind(
        "trade_finance_not_peril_excluded_bit",
        sub_expr(
            const_expr(1),
            signal_expr("trade_finance_peril_excluded_bit"),
        ),
    )?;
    builder.constrain_boolean("trade_finance_not_peril_excluded_bit")?;

    append_boolean_and(
        &mut builder,
        "trade_finance_policy_eligible_pre_category_bit",
        signal_expr("trade_finance_within_period_bit"),
        signal_expr("trade_finance_covered_peril_supported_bit"),
    )?;
    append_boolean_and(
        &mut builder,
        "trade_finance_policy_eligible_pre_exclusion_bit",
        signal_expr("trade_finance_policy_eligible_pre_category_bit"),
        signal_expr("trade_finance_not_peril_excluded_bit"),
    )?;
    append_boolean_and(
        &mut builder,
        "trade_finance_policy_eligible_bit",
        signal_expr("trade_finance_policy_eligible_pre_exclusion_bit"),
        signal_expr("trade_finance_claim_category_present_bit"),
    )?;
    ...
    let coverage_commitment = builder.append_poseidon_hash(
        ...
        [
            ...
            signal_expr("trade_finance_policy_eligible_bit"),
            signal_expr("trade_finance_within_period_bit"),
            signal_expr("trade_finance_excluded_peril_count"),
        ],
    )?;
    ...
    for aggregate in [
        ...
        "trade_finance_total_replacement_amount",
    ...
    builder.private_signal("trade_finance_policy_mismatch_score")?;
    builder.bind(
        "trade_finance_policy_mismatch_score",
        add_expr(vec![
            mul_expr(
                sub_expr(
                    const_expr(1),
    ...
    builder.private_signal("trade_finance_fraud_evidence_score_raw")?;
    builder.bind(
        "trade_finance_fraud_evidence_score_raw",
        add_expr(vec![
            ...
            signal_expr("trade_finance_policy_mismatch_score"),
        ]),
    )?;
    append_pairwise_min_signal(
        &mut builder,
        "trade_finance_fraud_evidence_score",
        "trade_finance_fraud_evidence_score_raw",
        "__trade_finance_score_cap",
        TRADE_FINANCE_SCORE_CAP * 2,
        "trade_finance_fraud_evidence_score",
    ...
    let fraud_commitment = builder.append_poseidon_hash(
        "trade_finance_fraud_commitment",
        [
            const_expr(TRADE_FINANCE_DOMAIN_FRAUD),
            signal_expr("trade_finance_fraud_evidence_score"),
            signal_expr(&governance_array_name("public_disclosure_blinding", 0)),
            signal_expr(&governance_array_name("public_disclosure_blinding", 1)),
        ],
    )?;
    ...
    append_pairwise_min_signal(
        &mut builder,
        "trade_finance_covered_amount_before_deductible",
        "trade_finance_total_estimate_amount",
        "trade_finance_total_replacement_amount",
        TRADE_FINANCE_VALUE_BOUND * TRADE_FINANCE_VALUE_BOUND,
        "trade_finance_covered_amount_before_deductible",
    ...
    append_geq_comparator_bit(
        &mut builder,
        signal_expr("trade_finance_covered_amount_before_deductible"),
        signal_expr(&policy_array_name("deductible_schedule", 0)),
        "trade_finance_deductible_applies_bit",
        "trade_finance_deductible_applies_slack",
        TRADE_FINANCE_VALUE_BOUND * TRADE_FINANCE_VALUE_BOUND,
        "trade_finance_deductible_applies",
    )?;
    builder.private_signal("trade_finance_deductible_adjusted_amount")?;
    builder.bind(
        "trade_finance_deductible_adjusted_amount",
        select_expr(
            signal_expr("trade_finance_deductible_applies_bit"),
            sub_expr(
                signal_expr("trade_finance_covered_amount_before_deductible"),
                signal_expr(&policy_array_name("deductible_schedule", 0)),
            ),
    ...
    builder.private_signal("trade_finance_depreciation_raw")?;
    builder.bind(
        "trade_finance_depreciation_raw",
        mul_expr(
            signal_expr("trade_finance_deductible_adjusted_amount"),
            signal_expr(&policy_array_name("depreciation_rule", 0)),
        ),
    )?;
    builder.append_exact_division_constraints(
        signal_expr("trade_finance_depreciation_raw"),
        signal_expr("__trade_finance_scale"),
        "trade_finance_depreciation_amount",
        "trade_finance_depreciation_remainder",
        "trade_finance_depreciation_slack",
        &BigInt::from(TRADE_FINANCE_FIXED_POINT_SCALE),
        "trade_finance_depreciation",
    )?;
    builder.private_signal("trade_finance_depreciation_adjusted_amount")?;
    builder.bind(
        "trade_finance_depreciation_adjusted_amount",
        sub_expr(
            signal_expr("trade_finance_deductible_adjusted_amount"),
            signal_expr("trade_finance_depreciation_amount"),
        ),
    )?;
    append_pairwise_min_signal(
        &mut builder,
        "trade_finance_capped_payout_amount",
        "trade_finance_depreciation_adjusted_amount",
    ...
    builder.bind(
        ...
        mul_expr(
            signal_expr("trade_finance_capped_payout_amount"),
    ...
    append_geq_comparator_bit(
        ...
        "trade_finance_reinsurer_attached_bit",
        "trade_finance_reinsurer_attached_slack",
        TRADE_FINANCE_VALUE_BOUND * TRADE_FINANCE_VALUE_BOUND,
        "trade_finance_reinsurer_attached",
    )?;
    builder.private_signal("trade_finance_reinsurer_attachment_excess")?;
    builder.bind(
        "trade_finance_reinsurer_attachment_excess",
        select_expr(
            signal_expr("trade_finance_reinsurer_attached_bit"),
            sub_expr(
                signal_expr("trade_finance_capped_payout_amount"),
                signal_expr(&policy_array_name("reinsurer_sharing_parameter", 1)),
            ),
    ...
    append_pairwise_min_signal(
        &mut builder,
        "trade_finance_reinsurer_share_base",
        "trade_finance_reinsurer_attachment_excess",
        &policy_array_name("reinsurer_sharing_parameter", 2),
        TRADE_FINANCE_VALUE_BOUND * TRADE_FINANCE_VALUE_BOUND,
        "trade_finance_reinsurer_share_base",
    )?;
    builder.private_signal("trade_finance_reinsurer_share_raw")?;
    builder.bind(
        "trade_finance_reinsurer_share_raw",
        mul_expr(
            signal_expr("trade_finance_reinsurer_share_base"),
            signal_expr(&policy_array_name("reinsurer_sharing_parameter", 0)),
        ),
    )?;
    builder.append_exact_division_constraints(
        signal_expr("trade_finance_reinsurer_share_raw"),
        signal_expr("__trade_finance_scale"),
        "trade_finance_reinsurer_share_amount",
        "trade_finance_reinsurer_share_remainder",
        "trade_finance_reinsurer_share_slack",
        &BigInt::from(TRADE_FINANCE_FIXED_POINT_SCALE),
        "trade_finance_reinsurer_share",
    )?;

    let payout_commitment_inner = builder.append_poseidon_hash(
        "trade_finance_payout_commitment_inner",
        [
            const_expr(TRADE_FINANCE_DOMAIN_PAYOUT),
            signal_expr("trade_finance_capped_payout_amount"),
    ...
    append_geq_comparator_bit(
        ...
        "trade_finance_fraud_threshold_hit_bit",
        "trade_finance_fraud_threshold_hit_slack",
        TRADE_FINANCE_SCORE_CAP,
        "trade_finance_fraud_threshold_hit",
    )?;
    append_geq_comparator_bit(
        &mut builder,
        signal_expr("trade_finance_capped_payout_amount"),
        signal_expr(&governance_name("manual_review_threshold")),
        "trade_finance_manual_review_payout_hit_bit",
        "trade_finance_manual_review_payout_hit_slack",
        TRADE_FINANCE_VALUE_BOUND * TRADE_FINANCE_VALUE_BOUND,
        "trade_finance_manual_review_payout_hit",
    )?;
    append_geq_comparator_bit(
        &mut builder,
        signal_expr("trade_finance_reserve_amount"),
    ...
    append_boolean_or(
        ...
        signal_expr("trade_finance_manual_review_payout_hit_bit"),
        signal_expr("trade_finance_manual_review_reserve_hit_bit"),
    )?;
    builder.private_signal("trade_finance_policy_ineligible_bit")?;
    builder.bind(
        "trade_finance_policy_ineligible_bit",
        sub_expr(
            const_expr(1),
            signal_expr("trade_finance_policy_eligible_bit"),
        ),
    )?;
    builder.constrain_boolean("trade_finance_policy_ineligible_bit")?;
    ...
    builder.private_signal("trade_finance_not_fraud_review_bit")?;
    builder.bind(
        "trade_finance_not_fraud_review_bit",
        sub_expr(
            const_expr(1),
            signal_expr("trade_finance_fraud_threshold_hit_bit"),
        ),
    )?;
    builder.constrain_boolean("trade_finance_not_fraud_review_bit")?;

    builder.private_signal("trade_finance_action_deny_policy_bit")?;
    builder.bind(
        "trade_finance_action_deny_policy_bit",
        signal_expr("trade_finance_policy_ineligible_bit"),
    )?;
    ...
    builder.bind(
        ...
        mul_expr(
            signal_expr("trade_finance_action_can_review_bit"),
            signal_expr("trade_finance_not_fraud_review_bit"),
        ),
    )?;
    ...
    append_geq_comparator_bit(
        ...
        signal_expr("trade_finance_capped_payout_amount"),
        const_expr(1),
        "trade_finance_payout_nonzero_bit",
        "trade_finance_payout_nonzero_slack",
        TRADE_FINANCE_VALUE_BOUND * TRADE_FINANCE_VALUE_BOUND,
        "trade_finance_payout_nonzero",
    )?;
    builder.constrain_equal(
        signal_expr("eligible_for_midnight_settlement"),
        mul_expr(
            signal_expr("trade_finance_action_approve_bit"),
            signal_expr("trade_finance_payout_nonzero_bit"),
        ),
    )?;
...
pub fn build_trade_finance_settlement_binding_program() -> ZkfResult<Program> {
    let mut builder = ProgramBuilder::new(
        "trade_finance_trade_finance_settlement_binding_v1",
        TRADE_FINANCE_FIELD,
    );
    ...
    append_geq_comparator_bit(
        &mut builder,
        signal_expr("trade_finance_settlement_fraud_evidence_score"),
        signal_expr("trade_finance_settlement_dispute_threshold"),
        "trade_finance_settlement_hold_by_fraud_bit",
        "trade_finance_settlement_hold_by_fraud_slack",
        TRADE_FINANCE_SCORE_CAP,
        "trade_finance_settlement_hold_by_fraud",
    )?;
    append_geq_comparator_bit(
        &mut builder,
    ...
    append_boolean_or(
        ...
        signal_expr("trade_finance_settlement_hold_by_fraud_bit"),
    ...
    let settlement_commitment = builder.append_poseidon_hash(
        ...
        [
            signal_expr("trade_finance_settlement_payout_amount"),
            signal_expr("trade_finance_settlement_reserve_amount"),
            signal_expr("trade_finance_settlement_action_class_code"),
            signal_expr("trade_finance_settlement_claimant_destination_commitment"),
        ],
    )?;
    let settlement_commitment_outer = builder.append_poseidon_hash(
        ...
        [
            ...
            signal_expr("trade_finance_settlement_insurer_reserve_account_commitment"),
    ...
    let settlement_commitment_binding = builder.append_poseidon_hash(
        ...
        [
            ...
            signal_expr("trade_finance_settlement_claim_packet_commitment"),
            signal_expr("trade_finance_settlement_coverage_decision_commitment"),
    ...
    let repayment_completion_commitment = builder.append_poseidon_hash(
        ...
        [
            signal_expr("trade_finance_settlement_reinsurer_share_amount"),
            signal_expr("trade_finance_settlement_payout_amount"),
            signal_expr("trade_finance_settlement_reinsurer_participation_commitment"),
    ...
    let maturity_schedule_inner = builder.append_poseidon_hash(
        ...
        [
            ...
            signal_expr("trade_finance_settlement_policy_effective_timestamp"),
            signal_expr("trade_finance_settlement_incident_timestamp"),
            signal_expr("trade_finance_settlement_reported_timestamp"),
        ],
    )?;
    let maturity_schedule_outer = builder.append_poseidon_hash(
        ...
        [
            ...
            signal_expr("trade_finance_settlement_policy_expiration_timestamp"),
            signal_expr("trade_finance_settlement_blinding_0"),
            signal_expr("trade_finance_settlement_blinding_1"),
        ],
    )?;
...
pub fn build_trade_finance_disclosure_projection_program() -> ZkfResult<Program> {
    let mut builder = ProgramBuilder::new(
        "trade_finance_trade_finance_disclosure_projection_v1",
        TRADE_FINANCE_FIELD,
    );
    ...
    for role in [
        ...
        "trade_finance_disclosure_role_reinsurer",
        "trade_finance_disclosure_role_claimant",
        "trade_finance_disclosure_role_investigator",
    ] {
    ...
    builder.bind(
        ...
        add_expr(vec![
            ...
            signal_expr("trade_finance_disclosure_role_reinsurer"),
            signal_expr("trade_finance_disclosure_role_claimant"),
            signal_expr("trade_finance_disclosure_role_investigator"),
        ]),
    )?;
    builder.constrain_equal(
    ...
    builder.bind(
        ...
        add_expr(vec![
            mul_expr(
                signal_expr("trade_finance_disclosure_role_auditor"),
                signal_expr("trade_finance_disclosure_payout_commitment"),
            ),
            mul_expr(
                signal_expr("trade_finance_disclosure_role_regulator"),
            ...
            mul_expr(
                signal_expr("trade_finance_disclosure_role_reinsurer"),
                signal_expr("trade_finance_disclosure_claim_packet_commitment"),
            ),
            mul_expr(
                signal_expr("trade_finance_disclosure_role_claimant"),
            ...
            mul_expr(
                signal_expr("trade_finance_disclosure_role_investigator"),
                signal_expr("trade_finance_disclosure_fraud_score_commitment"),
            ),
        ]),
    )?;
    ...
    builder.bind(
        ...
        add_expr(vec![
            ...
            mul_expr(
                signal_expr("trade_finance_disclosure_role_investigator"),
                signal_expr("trade_finance_disclosure_coverage_decision_commitment"),
            ),
        ]),
    )?;
    builder.constrain_equal(
    ...
    let view_commitment_outer = builder.append_poseidon_hash(
        ...
        [
            ...
            signal_expr("trade_finance_disclosure_reinsurer_share_amount"),
...
pub fn build_trade_finance_duplicate_registry_handoff_program() -> ZkfResult<Program> {
    let mut builder = ProgramBuilder::new(
        "trade_finance_trade_finance_duplicate_registry_handoff_v1",
        TRADE_FINANCE_FIELD,
    );
    ...
    for index in 0..PRIVATE_TRADE_FINANCE_MAX_DIGESTS {
        builder.append_exact_division_constraints(
            signal_expr(&format!("trade_finance_shard_claim_commitment_{index}")),
            signal_expr("trade_finance_shard_shard_count"),
    ...
    let batch_commitment = builder.append_poseidon_hash(
        ...
        [
            ...
            signal_expr("trade_finance_shard_claim_commitment_0"),
            signal_expr("trade_finance_shard_claim_commitment_1"),
            signal_expr("trade_finance_shard_claim_commitment_2"),
        ],
    )?;
    let batch_commitment_outer = builder.append_poseidon_hash(
        ...
        [
            ...
            signal_expr("trade_finance_shard_claim_commitment_3"),
...
fn compute_core_support_values(
    ...
    write_geq_support(
        &mut values,
        "trade_finance_fraud_evidence_score_geq_bit",
        "trade_finance_fraud_evidence_score_geq_slack",
        &BigInt::from(
            duplication_score
                .saturating_add(vendor_score)
                .saturating_add(chronology_score)
    ...
    let fraud_evidence_score_commitment = write_poseidon_hash_support(
        ...
        "trade_finance_fraud_commitment",
        [
            &BigInt::from(TRADE_FINANCE_DOMAIN_FRAUD),
    ...
    write_geq_support(
        &mut values,
        "trade_finance_covered_amount_before_deductible_geq_bit",
        "trade_finance_covered_amount_before_deductible_geq_slack",
    ...
    let depreciation_amount = depreciation_raw / TRADE_FINANCE_FIXED_POINT_SCALE;
    ...
    write_geq_support(
        &mut values,
        "trade_finance_capped_payout_amount_geq_bit",
        "trade_finance_capped_payout_amount_geq_slack",
    ...
    write_geq_support(
        &mut values,
        "trade_finance_reinsurer_share_base_geq_bit",
        "trade_finance_reinsurer_share_base_geq_slack",
    ...
    let reinsurer_share_amount = reinsurer_share_raw / TRADE_FINANCE_FIXED_POINT_SCALE;

    let payout_amount_commitment = write_poseidon_hash_support(
        &mut values,
        "trade_finance_payout_commitment_inner",
        [
            &BigInt::from(TRADE_FINANCE_DOMAIN_PAYOUT),
            &BigInt::from(payout_amount),
    ...
    let action_class = if deny_policy_bit {
        ClaimsActionClassV1::RejectForRuleFailure
    } else if deny_inconsistency_bit {
        ClaimsActionClassV1::RejectForInconsistency
    } else if escalate_bit {
        ClaimsActionClassV1::EscalateForRiskReview
    } else if manual_review_bit {
        ClaimsActionClassV1::ApproveWithManualReview
    } else {
        ClaimsActionClassV1::Approve
    };
...
pub(crate) fn trade_finance_claim_decision_witness_from_inputs(
    request: &TradeFinancePrivateInputsV1,
...
fn compute_trade_finance_settlement_binding_values(
    ...
    write_value(
        &mut values,
        "trade_finance_settlement_claim_packet_commitment",
        core.claim_packet_commitment.clone(),
    );
    write_value(
        &mut values,
        "trade_finance_settlement_coverage_decision_commitment",
        core.coverage_decision_commitment.clone(),
    );
    write_value(
        &mut values,
        "trade_finance_settlement_fraud_evidence_score",
        core.fraud_evidence_score,
    );
    write_value(
        &mut values,
        "trade_finance_settlement_payout_amount",
        core.payout_amount,
    );
    write_value(
        &mut values,
    ...
    write_value(
        &mut values,
        "trade_finance_settlement_reinsurer_share_amount",
    ...
    write_value(
        &mut values,
        "trade_finance_settlement_claimant_destination_commitment",
        request
            .settlement_governance
            .claimant_payout_destination_commitment,
    );
    write_value(
        &mut values,
        "trade_finance_settlement_insurer_reserve_account_commitment",
        request
            .settlement_governance
            .insurer_reserve_account_commitment,
    );
    write_value(
        &mut values,
        "trade_finance_settlement_reinsurer_participation_commitment",
        request
            .settlement_governance
            .reinsurer_participation_commitment,
    );
    ...
    write_value(
        &mut values,
        "trade_finance_settlement_policy_effective_timestamp",
        request.policy.policy_effective_timestamp,
    );
    write_value(
        &mut values,
        "trade_finance_settlement_policy_expiration_timestamp",
        request.policy.policy_expiration_timestamp,
    );
    write_value(
        &mut values,
        "trade_finance_settlement_incident_timestamp",
        request.claim_event.incident_timestamp,
    );
    write_value(
        &mut values,
        "trade_finance_settlement_reported_timestamp",
        request.claim_event.reported_timestamp,
    );
...
pub(crate) fn trade_finance_trade_finance_settlement_binding_witness_from_inputs(
    request: &TradeFinancePrivateInputsV1,
    core: &ClaimsCoreComputation,
...
fn compute_trade_finance_disclosure_projection_values(
    ...
    write_value(
        &mut values,
        "trade_finance_disclosure_claim_packet_commitment",
        core.claim_packet_commitment.clone(),
    );
    write_value(
        &mut values,
        "trade_finance_disclosure_coverage_decision_commitment",
        core.coverage_decision_commitment.clone(),
    );
    write_value(
        &mut values,
    ...
    write_value(
        &mut values,
        "trade_finance_disclosure_fraud_score_commitment",
        core.fraud_evidence_score_commitment.clone(),
    );
    write_value(
        &mut values,
        "trade_finance_disclosure_payout_commitment",
    ...
    write_value(
        &mut values,
        "trade_finance_disclosure_reinsurer_share_amount",
        core.reinsurer_share_amount,
    );
    write_value(
        &mut values,
...
pub(crate) fn trade_finance_trade_finance_disclosure_projection_witness_from_inputs(
    request: &TradeFinancePrivateInputsV1,
    core: &ClaimsCoreComputation,
    role_code: u64,
...
fn compute_batch_shard_values(
    ...
    for (index, commitment) in commitments.iter().enumerate() {
        write_value(
            &mut values,
            format!("trade_finance_shard_claim_commitment_{index}"),
            commitment.clone(),
        );
...
pub(crate) fn trade_finance_trade_finance_duplicate_registry_handoff_witness_from_commitments(
    commitments: &[BigInt; PRIVATE_TRADE_FINANCE_MAX_DIGESTS],
...
pub fn private_trade_finance_settlement_sample_inputs() -> TradeFinancePrivateInputsV1 {
    let mut sample = TradeFinancePrivateInputsV1 {
        policy: TradeFinancePolicyDataV1 {
            policy_id_hash: 1001,
        ...
        claim_event: TradeFinanceClaimEventDataV1 {
            claim_id_hash: 2002,
        ...
        evidence: TradeFinanceEvidenceDataV1 {
        ...
        analysis_inputs: TradeFinanceConsistencyFraudInputsV1 {
        ...
        settlement_governance: TradeFinanceSettlementGovernanceInputsV1 {
...
pub fn private_trade_finance_settlement_policy_denial_inputs() -> TradeFinancePrivateInputsV1 {
    let mut sample = private_trade_finance_settlement_sample_inputs();
...
pub fn private_trade_finance_settlement_showcase() -> ZkfResult<TemplateProgram> {
    Ok(TemplateProgram {
        ...
        violation_inputs: flatten_private_inputs(&private_trade_finance_settlement_violation_inputs())?,
        description: "Private trade finance and settlement showcase for property and casualty insurance claim consistency, scoring, payout, reserve, and settlement binding.",
    })
}
...
pub use export::{
    ...
    APP_ID as PRIVATE_TRADE_FINANCE_SETTLEMENT_APP_ID,
};

pub type TradeFinanceActionClassV1 = ClaimsActionClassV1;
pub type TradeFinanceCoreDecisionComputation = ClaimsCoreComputation;
pub type TradeFinanceDuplicateRegistryComputation = ClaimsShardComputation;

impl ClaimsCoreComputation {
...
pub fn private_trade_finance_settlement_reject_for_rule_failure_inputs(
    ...
    private_trade_finance_settlement_policy_denial_inputs()
}
...
pub fn trade_finance_decision_witness_from_inputs(
    ...
    trade_finance_claim_decision_witness_from_inputs(request)
}
...
pub(crate) fn trade_finance_settlement_binding_witness_from_inputs(
    ...
    trade_finance_trade_finance_settlement_binding_witness_from_inputs(request, core)
}
...
pub(crate) fn trade_finance_disclosure_projection_witness_from_inputs(
    ...
    trade_finance_trade_finance_disclosure_projection_witness_from_inputs(request, core, role_code)
}

pub(crate) fn trade_finance_duplicate_registry_handoff_witness_from_commitments(
    commitments: &[BigInt; 4],
) -> ZkfResult<(Witness, ClaimsShardComputation)> {
    trade_finance_trade_finance_duplicate_registry_handoff_witness_from_commitments(commitments)
}
...
mod tests {
    ...
    fn assert_action(request: &TradeFinancePrivateInputsV1, expected: ClaimsActionClassV1) {
        let program = build_trade_finance_decision_core_program().expect("program");
        let (witness, computation) =
            trade_finance_claim_decision_witness_from_inputs(request).expect("witness");
    ...
    #[test]
    fn trade_finance_policy_denial_fixture_is_valid() {
        assert_action(
            &private_trade_finance_settlement_policy_denial_inputs(),
            ClaimsActionClassV1::RejectForRuleFailure,
        );
    ...
    #[test]
    fn trade_finance_trade_finance_settlement_binding_matches_core_commitment() {
        let request = private_trade_finance_settlement_sample_inputs();
        let (core_witness, core) =
            trade_finance_claim_decision_witness_from_inputs(&request).expect("core");
        check_constraints(
    ...
    fn trade_finance_settlement_binding_emits_dedicated_fee_and_maturity_commitments() {
        ...
        let request = private_trade_finance_settlement_sample_inputs();
        let (_, core) = trade_finance_claim_decision_witness_from_inputs(&request).expect("core");
        ...
        let fee_amount = BigInt::from(core.reinsurer_share_amount);
        let settlement_blinding_0 =
            BigInt::from(request.settlement_governance.settlement_blinding_values[0]);
        let settlement_blinding_1 =
            BigInt::from(request.settlement_governance.settlement_blinding_values[1]);
        let public_blinding_1 = BigInt::from(
            request
                .settlement_governance
                .public_disclosure_blinding_values[1],
        );
    ...
    #[test]
    fn trade_finance_trade_finance_disclosure_projection_is_deterministic() {
        let request = private_trade_finance_settlement_sample_inputs();
        let (_, core_a) = trade_finance_claim_decision_witness_from_inputs(&request).expect("core");
        let (_, disclosure_a) =
            trade_finance_trade_finance_disclosure_projection_witness_from_inputs(
                &request, &core_a, 2,
            )
            .expect("disclosure");
        let (_, disclosure_b) =
    ...
    #[test]
    fn trade_finance_trade_finance_duplicate_registry_handoff_is_valid() {
        let request = private_trade_finance_settlement_sample_inputs();
        let (_, core_a) = trade_finance_claim_decision_witness_from_inputs(&request).expect("core");
        let commitments = [
        ...
        let (witness, _) =
            trade_finance_trade_finance_duplicate_registry_handoff_witness_from_commitments(
                &commitments,
            )
    ...
    fn trade_finance_sample_hypernova_prepared_witness_reports_pasta_overflow_values() {
        let (witness, _) = trade_finance_claim_decision_witness_from_inputs(
            &private_trade_finance_settlement_sample_inputs(),
        )
        .expect("witness");
        let program = build_trade_finance_decision_core_program().expect("program");
