use super::{
    ...
    build_trade_finance_settlement_binding_program, flatten_private_inputs,
    ...
    trade_finance_settlement_binding_witness_from_inputs, ClaimsActionClassV1,
    ClaimsCoreComputation, ClaimsSettlementComputation, ClaimsShardComputation,
    TradeFinancePrivateInputsV1, TradeFinancePublicOutputsV1,
};
...
use zkf_backends::{
    ...
    with_proof_seed_override, with_setup_seed_override, BackendSelection,
};
use zkf_core::{
    check_constraints, BackendKind, CompiledProgram, FieldId, Program, ProofArtifact, Witness,
    ZkfError, ZkfResult,
};
use zkf_runtime::{
...
fn action_class_label(action: ClaimsActionClassV1) -> &'static str {
    match action {
        ClaimsActionClassV1::Approve => "approve",
        ClaimsActionClassV1::ApproveWithManualReview => "approve_with_manual_review",
        ClaimsActionClassV1::EscalateForRiskReview => "escalate_for_risk_review",
        ClaimsActionClassV1::RejectForRuleFailure => "reject_for_rule_failure",
        ClaimsActionClassV1::RejectForInconsistency => "reject_for_inconsistency",
    }
}
...
fn render_public_outputs(
    ...
    TradeFinancePublicOutputsV1 {
        invoice_packet_commitment: bigint_string(&core.claim_packet_commitment),
        eligibility_commitment: bigint_string(&core.coverage_decision_commitment),
        consistency_score_commitment: bigint_string(&core.consistency_score_commitment),
        duplicate_financing_risk_commitment: bigint_string(&core.fraud_evidence_score_commitment),
        approved_advance_commitment: bigint_string(&core.payout_amount_commitment),
...
fn midnight_flow_typescript(entries: &[MidnightFlowCallEntry]) -> ZkfResult<String> {
    ...
    Ok(format!(
        "export const TRADE_FINANCE_TRUTH_SETTLEMENT_FLOW = {rendered} as const;\n"
    ))
}
...
"#,
    ...
    let mut flow_entries = vec![
        MidnightFlowCallEntry {
            ...
            inputs: json!({
                "invoicePacketCommitment": bigint_string(&core.claim_packet_commitment),
                "eligibilityCommitment": bigint_string(&core.coverage_decision_commitment),
        ...
        MidnightFlowCallEntry {
            ...
            inputs: json!({
                ...
                "approvedAdvanceCommitment": bigint_string(&core.payout_amount_commitment),
    ...
    vec![
        ...
        (
            ...
            vec![
                ...
                format!("The duplicate-financing risk lane remains rule based and fully explainable. The duplicate-financing risk score in this run is `{}` and it is not treated as an opaque model verdict. Instead it only participates in manual-review and risk-review gating according to the encoded governance thresholds.", core.fraud_evidence_score),
...
fn build_engineering_report(
    ...
    let midnight_contracts = [
        ("financing_request_registration", "binds the public financing-request registration event to the on-chain settlement lifecycle without disclosing raw supplier, buyer, invoice, or financing-policy evidence"),
        ("settlement_authorization", "binds the approved advance, reserve/holdback, maturity schedule, and settlement finality flag to the attested decision state"),
        ("dispute_hold", "captures dispute-hold transitions as explicit public state changes when the verified decision state is not settlement-final"),
        ("disclosure_access", "anchors the role-coded disclosure commitment and makes selective disclosure auditable"),
        ("repayment_completion", "captures repayment-completion commitments for the buyer repayment path"),
        ("supplier_receipt_confirmation", "lets the supplier acknowledge the settlement instruction without revealing unnecessary invoice-financing state"),
    ];
...
mod export_tests {
    ...
    fn midnight_contract_package_uses_field_commitments_and_complete_flows() {
        ...
        assert!(financing_request_registration
