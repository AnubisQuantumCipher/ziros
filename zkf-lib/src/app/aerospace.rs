...
fn insert_state(inputs: &mut WitnessInputs, prefix: &str, state: &RigidBodyStateV1) {
    ...
    insert_string_array(inputs, &format!("{prefix}_angular_rate"), &state.angular_rate);
}
...
fn bind_commitment_fold(
    ...
    builder.bind_labeled(target, commitment_fold_expr(inputs)?, Some(label.to_string()))?;
    Ok(())
}
...
fn build_gnc_6dof_core_program_internal(steps: usize) -> ZkfResult<Program> {
    ...
    let mut builder = ProgramBuilder::new(
        format!("gnc_6dof_core_{steps}_steps_v1"),
        FieldId::Bn254,
    );
    ...
    builder.metadata_entry("backend_pattern", "monte-carlo=plonky3;folding=nova-hypernova;wrap=groth16")?;
    ...
    add_surrogate_band_lookup(&mut builder, "gnc_surrogate_bands", &sample_surrogate_rows())?;
...
pub fn tower_catch_geometry_showcase() -> ZkfResult<TemplateProgram> {
    let sample_inputs = WitnessInputs::from([
        ...
        ("arm_clearance_floor".to_string(), field(decimal_scaled("3"))),
        ("closing_speed_limit".to_string(), field(decimal_scaled("1"))),
    ]);
    let mut violation_inputs = sample_inputs.clone();
    violation_inputs.insert(
        "arm_clearance".to_string(),
        field(decimal_scaled("2")),
    );
    Ok(TemplateProgram {
...
pub fn barge_terminal_profile_showcase() -> ZkfResult<TemplateProgram> {
    let sample_inputs = WitnessInputs::from([
        ...
        ("terminal_velocity".to_string(), field(decimal_scaled("0.9"))),
        ("lateral_clearance".to_string(), field(decimal_scaled("5"))),
        ("deck_heave_limit".to_string(), field(decimal_scaled("1"))),
        ("deck_velocity_limit".to_string(), field(decimal_scaled("1"))),
        ("terminal_velocity_limit".to_string(), field(decimal_scaled("1"))),
        ("lateral_clearance_floor".to_string(), field(decimal_scaled("4"))),
    ]);
    let mut violation_inputs = sample_inputs.clone();
    violation_inputs.insert(
...
fn build_planetary_terminal_profile_program() -> ZkfResult<Program> {
    ...
    builder.constrain_leq("slope_margin", signal_expr("slope"), signal_expr("slope_limit"), bits)?;
    builder.constrain_geq(
        "dust_margin",
...
pub fn planetary_terminal_profile_showcase() -> ZkfResult<TemplateProgram> {
    let sample_inputs = WitnessInputs::from([
        ...
        ("terminal_velocity".to_string(), field(decimal_scaled("0.7"))),
        ...
        ("dust_clearance_floor".to_string(), field(decimal_scaled("4"))),
    ]);
    let mut violation_inputs = sample_inputs.clone();
...
fn build_gust_robustness_batch_program_internal(samples: usize) -> ZkfResult<Program> {
    ...
    builder.metadata_entry("domain", "aerospace")?;
    builder.metadata_entry("production_monte_carlo_target", &STARSHIP_MONTE_CARLO_PRODUCTION_TARGET_SAMPLES.to_string())?;
    ...
    for name in ["gust_bound", "admitted_failures", "replan_latency_ms", "max_replan_latency_ms"] {
        builder.constrain_range(name, bits)?;
    }
    for gust in &gusts {
    ...
    let mut commitment_inputs = gusts.iter().map(|name| signal_expr(name)).collect::<Vec<_>>();
    commitment_inputs.extend([
        signal_expr("gust_bound"),
        signal_expr("admitted_failures"),
        signal_expr("replan_latency_ms"),
...
pub fn private_starship_flip_catch_inputs_from_request(
    ...
    insert_decimal(
        &mut inputs,
        "gust_bound",
        &request.monte_carlo.gust_bound,
    );
    insert_decimal(
        &mut inputs,
    ...
    insert_commitment(
        ...
        &request.distributed_proving.deterministic_partition_manifest_commitment,
    );
    insert_commitment(
        &mut inputs,
    ...
    for descriptor in ordered_descriptors {
        ...
        insert_commitment(&mut inputs, &format!("{prefix}_commitment"), &descriptor.interface_commitment);
        insert_decimal(&mut inputs, format!("{prefix}_residual_bound"), &descriptor.residual_bound);
        insert_u64(
            &mut inputs,
...
fn build_private_starship_flip_catch_program_internal(
    ...
    builder.metadata_entry("domain", "aerospace")?;
    builder.metadata_entry("landing_profile_default", &landing_profile_code(profile).to_string())?;
    builder.metadata_entry("production_trusted_setup", "imported-crs-only")?;
    builder.metadata_entry("neural_engine_role", "advisory-only")?;
    builder.metadata_entry("distributed_transport", "tcp-counted;rdma-follow-on-unshipped")?;
    builder.metadata_entry("distributed_cluster_target", "128")?;
    builder.metadata_entry("backend_pattern", "batch=plonky3;long-horizon=nova-hypernova;final-wrap=groth16")?;
    ...
    builder.metadata_entry("production_batch_target", &STARSHIP_MONTE_CARLO_PRODUCTION_TARGET_SAMPLES.to_string())?;
    builder.metadata_entry("normalization_scale", &science_scale_string())?;
    ...
    let crs_ceremony_commitment = builder.private_input_array("imported_crs_ceremony_commitment", 4)?;
    let crs_program_binding_commitment = builder.private_input_array("imported_crs_program_binding_commitment", 4)?;
    ...
    add_surrogate_band_lookup(&mut builder, "starship_surrogate_bands", &sample_surrogate_rows())?;
    for kind in TEAM_KIND_ORDER {
        let slug = team_kind_slug(kind);
        constrain_surrogate_band_lookup(
            &mut builder,
...
mod tests {
    ...
    fn starship_request_inputs_fail_closed_on_missing_team_subgraph() {
        let mut request = private_starship_flip_catch_sample_request(
            LandingInterfaceProfileV1::TowerCatch,
            4,
        );
    ...
    fn starship_request_inputs_encode_profile_and_transport_flags() {
        ...
        assert_eq!(inputs.get("tcp_transport_enforced"), Some(&FieldElement::ONE));
